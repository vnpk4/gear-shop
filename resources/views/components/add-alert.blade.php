<div>
    @if(session('success'))
        <div class="mb-8 p-4 bg-green-100 border border-green-400 text-green-700 rounded-xl flex items-center justify-between shadow-sm">
            ...
            @if(Route::has('customer.cart.index'))
                <a href="{{ route('customer.cart.index') }}" class="text-sm bg-green-600 hover:bg-green-700 text-white py-2 px-4 rounded-lg font-semibold transition-colors whitespace-nowrap ml-4">
                    Xem giỏ hàng
                </a>
            @endif
        </div>
